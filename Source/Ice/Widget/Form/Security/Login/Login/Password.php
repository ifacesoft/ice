...
class Login_Password extends Widget_Form_Security_Login
{
    ...
    {
        foreach (Query::getBuilder(Account::getClass())->eq(['login' => $this->getValues()['login']])->executeSelect(['password', 'user__fk'])->getRows() as $accountRow) {
            if (password_verify($this->validate()['password'], $accountRow['password'])) {
                ...
                $_SESSION['roleNames'] = Query::getBuilder(User_Role_Link::getClass())
                    ...
                    ->executeSelect('role_name')->getColumn();
                return;
            }
        }
