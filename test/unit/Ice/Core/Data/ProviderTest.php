...
class DataProviderTest extends TestCase
{
    ...
    {
        foreach (scandir(getSourceDir('Ice') . 'Ice/DataProvider', 1) as $dataProviderFile) {
            if (
                $dataProviderFile == '..' ||
                $dataProviderFile == '.' ||
                $dataProviderFile == 'Resource.php' ||
                $dataProviderFile == 'Request.php' ||
                $dataProviderFile == 'Router.php' ||
                $dataProviderFile == 'QueryResultRow.php' ||
            ...
            if (
                ...
                $dataProviderFile == 'Security.php' ||
                $dataProviderFile == 'Apc.php'
            ) {
                continue;
            }

            $this->assertEquals($dataProvider->set(['test' => '8'])['test'], $dataProvider->get('test'));

            if ($dataProviderFile != 'Resource.php') {
                $this->assertNull($dataProvider->get('dsadsaldlsadlsa'));
            }

            if ($dataProviderFile == 'Resource.php' || $dataProviderFile == 'App.php') {
                continue;
            }
